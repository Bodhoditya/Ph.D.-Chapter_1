...
N=100
N1=200
S=matrix(c(runif(900, min = 0, max = 0.5) ),30,30)
Sigma=t(S)%*%S
beta=matrix(0,K,nsim)
for (i in 1:nsim) {
  R=mvrnorm(N,c(rep(0, times = 30)),Sigma)
  X=cbind(matrix(1,N,1),R[,c(1:30)])
...
for (i in 1:nsim) {
  R1=mvrnorm(N1,c(rep(0, times = 30)),Sigma)
  X1=cbind(matrix(1,N1,1),R1[,c(1:30)])
  eps1=rnorm(N1, mean = 0, sd = 1)
  y1=X1%*%est$`beta_ols (average)`+eps1
  y11=as.matrix(y1,ncol=1,`colnames<-`(y))
  y_mean=mean(y1)
  data=cbind(y11,R1[,c(1:30)])
  X11=as.matrix(data)
